...
export class Operation<T> implements IOperation<T> {
  ...
  private _copiedData?: T = undefined;
  ...
  public _setData = (data: T) => {
    this._data = data;
  };

  public _setCopiedData = (data: T) => {
  ...
  run = async (request: any) => {
    ...
    try {
      ...
      this.setSuccess();
      this._setData(response as T);
      this._setCopiedData(response as T);
      return response;
    } catch (e: any) {
  ...
  public get copiedData() {
    return this._copiedData || this._defaultData;
  }
